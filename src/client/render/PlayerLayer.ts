...
// Voxel models generated by scripts/voxel-models (bun scripts/voxel-models/build.ts).
...
// Clip names from scripts/voxel-models/animations.ts. Death has no clip: the model bursts into
// its voxels instead (voxelBurst.ts).
...
// What the death burst needs from the World: the pull seed and the log that timestamps deaths.
export type DeathSource = { seed: number; log: readonly LogEntry[] };

// Sim time of the player's latest death at or before `time` (the log records it in the tick).
...
export class PlayerLayer {
  ...
  private syncAnimation(player: Player, roots: AbstractMesh[], time: number, deaths: DeathSource): void {
    ...
    if (!player.alive) {
      ...
      // The burst is drawn from sim time since death, so pauses freeze it and seeks land on the
      // right frame; outside its window the player is simply gone.
