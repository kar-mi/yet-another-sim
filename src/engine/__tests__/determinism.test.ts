...
const determinismCases: { name: string; make: (seed: number) => World; drive: (w: World) => Intents; ticks: number }[] = [
  ...
  },
  {
    ...
    make: seed => createWorld(buildRaid({
      ...
      events: [{ type: "chain", t: 0.1, name: "Chain", pairs: [["m1", "ot"]], telegraph: 0.5, breakWindow: 5, breakDistance: 12, breakDamage: 40, damageType: "magical", debuff: "chain_bond" }],
    }), seed),
    ...
    make: seed => withPlayerEffect(
      createWorld(applyBotPatterns(
        ...
        loadBotPatterns({ players: {}, solvers: { generic: [
          ...
          { when: { plant: "up up", plantSlot: 0 }, spot: { x: -18, z: 0 } },
          { when: { plant: "up up", plantSlot: 1 }, spot: { x: 0, z: -18 } },
        ] } }),
      ), seed),
      "mt",
