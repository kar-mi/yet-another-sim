...
const SKIP = /(dmath\.ts|rng\.ts|logger\.ts|effects\/sampling\.ts|\.test\.ts)$|__tests__|\/babylon\//;
const SIM_DIRS = ["src/engine", "src/shared", "src/status", "src/model", "src/arena", "src/effects"];
...
test("simulation code uses only deterministic math", () => {
  const files = SIM_DIRS.flatMap(tsFiles)
...
test("determinism guard classifies operations", () => {
  const banned = [
    ...
    "Math.expm1(x)", "Math.cbrt(x)", "Date.now()", "performance.now()", "x ** 2",
  ];
