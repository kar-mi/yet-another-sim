...
export function resolveEffectChecks(ctx: TickContext): PendingEffectCheck[] {
  ...
  for (const check of ctx.world.pendingEffectChecks) {
    ...
    for (const rule of check.checks) for (const player of ctx.players) {
      if (!player.alive || !player.effects.some(e => e.name === rule.carriers && isStatusActive(e, ctx.time))) continue;
      const [left, right] = rule.compare.map(group => player.effects.find(e => e.group === group && isStatusActive(e, ctx.time))?.name);
